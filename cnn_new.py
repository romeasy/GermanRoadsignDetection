...
#
# Note further that this code relies on the data being preprocessed by a notebook called 'preprocessing.ipynb'
...
class cNN:
    ...
    def __init__(self, _architecture, _img_shape=(28, 28), _kernel_shape=(5, 5), _learning_rate=0.001):
        ...
        self.n_classes = self.architecture[-1][1] #number of units in last layer
        
        print "Training network with the following parameters: "
        print "Learning Rate: " + str(self.learning_rate) + "\timg_shape: " + str(self.img_shape) + "\tkernel_shape: " + str(self.kernel_shape)
    ...
    def construct_model(self, images, labels, keep_prob):
        ...
        assert len(self.architecture) == len(self.weights)
        assert len(self.architecture) == len(self.biases)

        current_input = images

        for layer in xrange(self.architecture):
            if self.architecture[layer][0] == "conv":
                conv_out = tf.nn.relu(self.conv2d(current_input, self.weights[layer]) + self.biases[layer])
                current_output = self.max_pool_2x2(conv_out, k=4)
...
with open('train_data_yuv.pkl', 'rb') as train_handle:
    train_set, train_labels = pickle.load(train_handle)
with open('test_data_yuv.pkl', 'rb') as test_handle:
